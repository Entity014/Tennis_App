...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(504).message("Unsatisfiable Request (only-if-cached)").body(EMPTY_BODY).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        return response.newBuilder().body(new RealResponseBody(response.headers(), Okio.buffer(new Source() { // from class: okhttp3.internal.cache.CacheInterceptor.2
    ...
    private static boolean validate(Response response, Response response2) {
        ...
        Date date2 = response.headers().getDate(com.google.common.net.HttpHeaders.LAST_MODIFIED);
        return (date2 == null || (date = response2.headers().getDate(com.google.common.net.HttpHeaders.LAST_MODIFIED)) == null || date.getTime() >= date2.getTime()) ? false : true;
    }
    ...
    private static Headers combine(Headers headers, Headers headers2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!com.google.common.net.HttpHeaders.WARNING.equalsIgnoreCase(name) || !value.startsWith(IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE)) && (!isEndToEnd(name) || headers2.get(name) == null)) {
