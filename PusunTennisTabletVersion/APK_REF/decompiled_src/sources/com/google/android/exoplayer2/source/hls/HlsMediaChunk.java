...
final class HlsMediaChunk extends MediaChunk {
    ...
    public static HlsMediaChunk createInstance(HlsExtractorFactory hlsExtractorFactory, DataSource dataSource, Format format, long j, HlsMediaPlaylist hlsMediaPlaylist, HlsChunkSource.SegmentBaseHolder segmentBaseHolder, Uri uri, List<Format> list, int i, Object obj, boolean z, TimestampAdjusterProvider timestampAdjusterProvider, long j2, HlsMediaChunk hlsMediaChunk, byte[] bArr, byte[] bArr2, boolean z2, PlayerId playerId, CmcdHeadersFactory cmcdHeadersFactory) {
        ...
        if (cmcdHeadersFactory == null) {
            ...
            createHttpRequestHeaders = cmcdHeadersFactory.setChunkDurationUs(segmentBase.durationUs).createHttpRequestHeaders();
        }
        DataSpec build = new DataSpec.Builder().setUri(UriUtil.resolveToUri(hlsMediaPlaylist.baseUri, segmentBase.url)).setPosition(segmentBase.byteRangeOffset).setLength(segmentBase.byteRangeLength).setFlags(segmentBaseHolder.isPreload ? 8 : 0).setHttpRequestHeaders(createHttpRequestHeaders).build();
        ...
        if (segment != null) {
            ...
            if (cmcdHeadersFactory == null) {
                ...
                createHttpRequestHeaders2 = cmcdHeadersFactory.setObjectType(CmcdHeadersFactory.OBJECT_TYPE_INIT_SEGMENT).createHttpRequestHeaders();
            }
            dataSpec = new DataSpec.Builder().setUri(resolveToUri).setPosition(segment.byteRangeOffset).setLength(segment.byteRangeLength).setHttpRequestHeaders(createHttpRequestHeaders2).build();
        ...
        if (hlsMediaChunk != null) {
            ...
            boolean z6 = dataSpec == dataSpec2 || (dataSpec != null && dataSpec2 != null && dataSpec.uri.equals(hlsMediaChunk.initDataSpec.uri) && dataSpec.position == hlsMediaChunk.initDataSpec.position);
