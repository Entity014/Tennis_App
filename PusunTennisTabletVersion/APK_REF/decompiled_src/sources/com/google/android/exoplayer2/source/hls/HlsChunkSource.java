...
class HlsChunkSource {
    ...
    public void getNextChunk(long j, long j2, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        long j5 = j4;
        this.trackSelection.updateSelectedTrack(j, j5, resolveTimeToLiveEdgeUs, list, createMediaChunkIterators(hlsMediaChunk, j2));
        ...
        if (this.cmcdConfiguration == null) {
            ...
            CmcdHeadersFactory cmcdHeadersFactory = new CmcdHeadersFactory(this.cmcdConfiguration, this.trackSelection, j5, CmcdHeadersFactory.STREAMING_FORMAT_HLS, !hlsMediaPlaylist.hasEndTag);
            if (getIsMuxedAudioAndVideo()) {
                objectType = CmcdHeadersFactory.OBJECT_TYPE_MUXED_AUDIO_AND_VIDEO;
            } else {
    ...
    private Chunk maybeCreateEncryptionChunkFor(Uri uri, int i, boolean z, CmcdHeadersFactory cmcdHeadersFactory) {
        ...
        return new EncryptionKeyChunk(this.encryptionDataSource, new DataSpec.Builder().setUri(uri).setFlags(1).setHttpRequestHeaders(of).build(), this.playlistFormats[i], this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), this.scratchSpace);
    }
